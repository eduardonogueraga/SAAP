...
class SystemNoticesScreen extends StatefulWidget {
  final String? endpoint;
  final Map<String, dynamic>? initialFilters;
  const SystemNoticesScreen({super.key, this.endpoint, this.initialFilters});

  @override
  State<SystemNoticesScreen> createState() => _SystemNoticesScreenState();
}

class _SystemNoticesScreenState extends State<SystemNoticesScreen> {
  ...
  late String _endpoint; // full /system_notices
  ...
  void initState() {
    ...
    _endpoint = normalizedBase.endsWith('/system_notices')
        ? normalizedBase
        : '$normalizedBase/system_notices';
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      debugPrint('Error system_notices: $e');
  ...
  void _openFilters() {
    // tipo from data, procesado as 3-state dropdown, plus date range
    ...
    String procesadoSel = _filters.containsKey('procesado')
        ? (_filters['procesado'].toString() == '1' ? 'Procesadas' : 'No procesadas')
        : 'Todas';
              ...
              return Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    value: procesadoSel,
                    isExpanded: true,
                    decoration: const InputDecoration(labelText: 'Procesado'),
                    items: const [
                      DropdownMenuItem(value: 'Todas', child: Text('Todas')),
                      DropdownMenuItem(value: 'Procesadas', child: Text('Procesadas')),
                      DropdownMenuItem(value: 'No procesadas', child: Text('No procesadas')),
                    ],
                    onChanged: (v) => setModalState(() => procesadoSel = v ?? 'Todas'),
                  ),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            if (procesadoSel == 'Procesadas') newFilters['procesado'] = '1';
                            if (procesadoSel == 'No procesadas') newFilters['procesado'] = '0';
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < _items.length) {
            ...
            final String fechaFmt = (n['_fecha_fmt'] as String?) ?? _formatDate((n['fecha'] ?? '').toString());
            final String tipo = (n['tipo'] ?? '').toString();
            final int proc = _toInt(n['procesado']);
            final String desc = (n['desc'] ?? '').toString();

            final bool isAlarm = tipo.toLowerCase() == 'alarm';
            final IconData icon = isAlarm ? Icons.warning_amber : Icons.settings;
            final Color color = isAlarm ? Colors.red : Colors.blueGrey;
                    ...
                    Wrap(
                      ...
                      children: [
                        Chip(
                          ...
                          label: Text(proc == 1 ? 'Procesada' : 'Pendiente'),
