...
class NoticesScreen extends StatefulWidget {
  ...
  const NoticesScreen({super.key, this.endpoint, this.initialFilters});
...
class _NoticesScreenState extends State<NoticesScreen> {
  ...
  late String _endpoint; // full notices endpoint
  ...
  void initState() {
    ...
    _endpoint = normalizedBase.endsWith('/notices') ? normalizedBase : '$normalizedBase/notices';
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> newItems = result
        ...
        final isSms = tipo.contains('sms');
        final isCall = tipo.contains('llamada') || tipo.contains('call') || tipo == 'llamada';
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < _items.length) {
            ...
            final String tipo = (n['tipo'] ?? 'Desconocido').toString();
            final String telefono = (n['telefono'] ?? '').toString();
            final String fechaFmt = (n['_fecha_fmt'] as String?) ?? _formatDate((n['fecha'] ?? '').toString());

            final bool isSms = (n['_isSms'] as bool?) ?? false;
            final bool isCall = (n['_isCall'] as bool?) ?? false;

            final IconData icon = isSms
                ? Icons.sms
                : isCall
                    ? Icons.call
                    : Icons.notifications_none;
            final Color color = isSms
                ? Colors.indigo
                : isCall
                    ? Colors.teal
                    : Colors.blueGrey;
